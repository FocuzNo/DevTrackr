...
public sealed class TopicStatisticsConfiguration : IEntityTypeConfiguration<TopicStatistics>
{
    public void Configure(EntityTypeBuilder<TopicStatistics> builder)
    {
        builder.ToTable("topic_statistics");

        builder.HasKey(x => x.Id);

        builder.Property(x => x.Id).HasColumnName("id");
        builder.Property(x => x.UserId).HasColumnName("user_id");
        builder.Property(x => x.Topic).HasColumnName("topic").HasMaxLength(100);
        ...
        builder.HasIndex(x => new { x.UserId, x.Topic }).IsUnique();
    }
}
