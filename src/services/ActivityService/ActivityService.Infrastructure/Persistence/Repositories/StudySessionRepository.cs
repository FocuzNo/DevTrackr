...
public sealed class StudySessionRepository(ActivityDbContext dbContext) : IStudySessionRepository
{
    ...
    public Task<StudySession?> GetByIdAsync(
        Guid id,
        CancellationToken cancellationToken = default) =>
        dbContext.StudySessions.FirstOrDefaultAsync(x => x.Id == id, cancellationToken);
    ...
    public void Update(StudySession studySession) => dbContext.StudySessions.Update(studySession);

    public void Remove(StudySession studySession) => dbContext.StudySessions.Remove(studySession);
}
